...
def main(args):
    ...
    for epoch in range(0, args.epoch):
        ...
        if is_best:
            pruner.export_model(model_path=os.path.join(args.save_path,"best_prune_unet.pth.tar"), mask_path=os.path.join(args.save_path,'mask_prune_indexs.pth.tar'))
        state = {
            'epoch': epoch,
    ...
    args.model='unet'
    model_weight_path=os.path.join(args.save_path,"best_prune_unet.pth.tar")
    model=get_models(args)
