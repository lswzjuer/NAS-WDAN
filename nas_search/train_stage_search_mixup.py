    ...
    for sp in range(2):
        ...
        for epoch in range(0, sp_epoch):
            ...
            if (epoch + 1) % args.infer_epoch == 0:
                ...
                writer.add_scalar('stage{}_Val/mjc'.format(sp), vmjc, epoch)
...
def mixup_data2(x, y, alpha=1.0, use_cuda=True):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        lam = np.random.beta(alpha, alpha,3)
        print(lam)
        lam= softmax_func(lam)
        print(lam)
    else:
        lam = 1
    ...
    mixed_x = lam[0] * x + lam[1]* x[index1, :]+lam[2]*x[index2, :]
...
def train(args, train_queue, val_queue, model, criterion, optimizer_weight, optimizer_arch, train_arch):
    ...
    for step, (input, target,_) in tqdm(enumerate(train_queue)):
        ...
        mixup_images,target,perm_target,lam=mixup_data(input,target,alpha=args.alpha,use_cuda=args.use_cuda)
        ...
        if train_arch:
            ...
            mixup_input_search, target_search, perm_target_search, alam = mixup_data(input_search, target_search, alpha=args.alpha, use_cuda=args.use_cuda)
            ...
            perm_target_search=perm_target_search.view(perm_target_search.size(0), -1)
            torch.cuda.empty_cache()
