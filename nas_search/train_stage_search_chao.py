...
from utils import RefugeIndicatorsMetricBinary,AverageMeter
...
def main(args):
    ...
    for sp in range(2):
        ...
        optimizer_cellarch = torch.optim.Adam(model.arch_parameters()[:3], lr=args.arch_lr, betas=(0.5, 0.999),
                                            weight_decay=args.arch_weight_decay)

        optimizer_narch = torch.optim.Adam([model.arch_parameters()[-1]], lr=args.arch_lr, betas=(0.5, 0.999),
                                            weight_decay=args.arch_weight_decay)
        ...
        for epoch in range(0, sp_epoch):
            ...
            if epoch < args.arch_after:
                trainloss = train(args, train_queue, val_queue, model, criterion,optimizer_weight, optimizer_cellarch,optimizer_narch, train_arch=False)
            else:
                trainloss= train(args, train_queue, val_queue,model, criterion,optimizer_weight, optimizer_cellarch,optimizer_narch,train_arch=True)
            ...
            if (epoch + 1) % args.infer_epoch == 0:
                ...
                state = {
                    ...
                    'optimizer_narch': optimizer_narch.state_dict(),
...
def train(args, train_queue, val_queue, model, criterion, optimizer_weight, optimizer_cellarch,optimizer_narch, train_arch):
    w_tloss_recoder = AverageMeter()
    cell_tloss_recoder = AverageMeter()
    n_tloss_recoder = AverageMeter()
    model.train()
    for step, (input, target,_) in tqdm(enumerate(train_queue)):
        ...
        if train_arch:
            ...
            if args.deepsupervision:
                ...
                for index in range(len(archs_preds)):
                    asubtloss = criterion(archs_preds[index].view(archs_preds[index].size(0),-1), target_search.view(target_search.size(0),-1))
                    atloss += asubtloss
                atloss = atloss * archs_preds[-1].size(0)
            else:
                atloss = criterion(archs_preds[-1].view(archs_preds[-1].size(0),-1), target_search.view(target_search.size(0),-1))
                atloss = atloss * archs_preds[-1].size(0)
            atloss.backward()
            if args.grad_clip:
                nn.utils.clip_grad_norm_(model.arch_parameters(), args.grad_clip)
            optimizer_cellarch.step()
            cell_tloss_recoder.update(atloss.item(), 1)
        ...
        if train_arch:
            ...
            if args.grad_clip:
                nn.utils.clip_grad_norm_(model.arch_parameters(), args.grad_clip)
            optimizer_narch.step()
            n_tloss_recoder.update(atloss.item(), 1)
            
    weight_loss_avg = w_tloss_recoder.avg
    if train_arch:
        cell_loss_avg = cell_tloss_recoder.avg
        n_loss_avg= n_tloss_recoder.avg
    else:
        cell_loss_avg = 0
        n_loss_avg= 0
    return weight_loss_avg,cell_loss_avg,n_loss_avg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', type=str,
                        default='/data/home/rileyliu/datasets/CHAO',
                        help='Model to train and evaluation')
    ...
    # lr
    parser.add_argument('--lr', type=float, default=0.001, help="weight parameters lr ")
    ...
    parser.add_argument('--arch_weight_decay', type=float, default=5e-3, help=" for arch parameters lr ")
